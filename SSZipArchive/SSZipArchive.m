      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    if ([delegate respondsToSelector:@selector(ssZipArchiveProgressEvent:total:)]) {
        [delegate ssZipArchiveProgressEvent:currentPosition total:fileSize];
    }
    ...
    do {
        ...
        @autoreleasepool {
            ...
            if ([delegate respondsToSelector:@selector(ssZipArchiveProgressEvent:total:)]) {
                [delegate ssZipArchiveProgressEvent:(NSInteger)currentPosition total:(NSInteger)fileSize];
            }
            ...
            // Message delegate
            if ([delegate respondsToSelector:@selector(ssZipArchiveDidUnzipFileAtIndex:totalFiles:archivePath:fileInfo:)]) {
                [delegate ssZipArchiveDidUnzipFileAtIndex:currentFileNumber totalFiles:(NSInteger)globalInfo.number_entry
                                            archivePath:path fileInfo:fileInfo];
            } else if ([delegate respondsToSelector: @selector(ssZipArchiveDidUnzipFileAtIndex:totalFiles:archivePath:unzippedFilePath:)]) {
                [delegate ssZipArchiveDidUnzipFileAtIndex: currentFileNumber totalFiles: (NSInteger)globalInfo.number_entry
    ...
    if (!canceled && [delegate respondsToSelector:@selector(ssZipArchiveProgressEvent:total:)]) {
        [delegate ssZipArchiveProgressEvent:fileSize total:fileSize];
    }
